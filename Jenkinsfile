    ...
    stages {
        ...
        stage('CD') {
            steps {
              ...
              sh "docker network create kaf"
              sh "docker run -d --name mongo_container -p 27017:27017 --net db mongo_image"
              sh "docker exec -ti mongo_container mongosh localhost:27017 mongoScript.js"
              sh "docker run -d -p 4000:5000 -e PORT=4000 -e db_url=mongo_container:27017 --net kaf --name=flaskapp flask_image"              sh "docker run -d -p 400:3000 mennahg/angular_image"
              sh "docker run -d -p 3000:80 -e flaskPort=4000 --net front --name=angular_container angular_image"
